...
// Server action
import { upsertShippingRate } from "@/actions/store";
import { Textarea } from "@/components/ui/textarea";

interface ShippingRateDetailsProps {
  data?: StoreShippingRateForCountryType;
  storeUrl: string;
  closeModal: () => void;
}

export default function ShippingRateDetails({ data, storeUrl, closeModal }: ShippingRateDetailsProps) {
  ...
  const form = useForm<z.infer<typeof ShippingRateFormSchema>>({
    ...
    defaultValues: {
      ...
      shippingFeePerItem: data?.shippingRate?.shippingFeePerItem
        ? data.shippingRate.shippingFeePerItem / 100
        : 0,
      shippingFeePerAdditionalItem:
        data?.shippingRate?.shippingFeePerAdditionalItem
          ? data.shippingRate.shippingFeePerAdditionalItem / 100
          : 0,
      shippingFeePerKg: data?.shippingRate?.shippingFeePerKg
        ? data.shippingRate.shippingFeePerKg / 100
        : 0,
      shippingFeeFixed: data?.shippingRate?.shippingFeeFixed
        ? data.shippingRate.shippingFeeFixed / 100
        : 0,
    ...
    if (data) {
      form.reset({
        ...
        shippingFeePerItem: data?.shippingRate?.shippingFeePerItem
          ? data.shippingRate.shippingFeePerItem / 100
          : 0,
        shippingFeePerAdditionalItem: data?.shippingRate
          ?.shippingFeePerAdditionalItem
          ? data.shippingRate.shippingFeePerAdditionalItem / 100
          : 0,
        shippingFeePerKg: data?.shippingRate?.shippingFeePerKg
  ...
  async function onSubmit(values: z.infer<typeof ShippingRateFormSchema>) {
    const response = await upsertShippingRate(storeUrl, values);
        ...
        if (response.fieldErrors) {
          Object.entries(response.fieldErrors).forEach(([field, messages]) => {
            if (messages && messages.length > 0) {
              form.setError(field as keyof z.infer<typeof ShippingRateFormSchema>, {
                message: messages[0],
              });
            }
          });
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-2xl">
            Store Shipping Details {data ? `for ${data.countryName}` : ""}
          </CardTitle>
        ...
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4 mt-8 border p-4 rounded-md">
                <FormField
                  control={form.control}
                ...
                <FormField
                  control={form.control}
                  name="shippingFeePerAdditionalItem"
                  render={({ field }) => (
                    <FormItem className="flex flex-col justify-between">
