...
export default function ProfileSidebar() {
  ...
  return (
    <div>
      ...
      <div className="py-3 inline-block w-full lg:w-[296px] min-h-72 bg-white">
        <div className="font-bold text-main-primary flex h-9 items-center px-4 whitespace-nowrap overflow-ellipsis overflow-hidden">
          Account
        </div>
        {/* Links */}
        {menu.map((item) => (
          <Link key={item.link} href={item.link}>
            <div
              className={cn(
                ...
                {
                  "bg-[#f5f5f5] border-l-6 border-orange-background":
                    item.link &&
                    (pathname === item.link ||
