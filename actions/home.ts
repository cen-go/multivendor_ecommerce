...
import { ProductSimpleVariantType, ProductSize, ProductType, ProductWithVariants, SimpleProduct, VariantImageType } from "@/lib/types";
...
export async function getHomeDataDynamic(params:Param[]): Promise<Record<string, SimpleProduct[] | ProductType[]>> {
  if (!Array.isArray(params) || params.length === 0) {
    throw new Error("Invalid input: params must be a non-empty array.");
  }

    // Define the mappings for proparty names to database fields
    ...
    if (type === "simple") {
      ...
      return products.map((product) => {
        ...
        const variantImages: VariantImageType[] = variants.map((variant) => ({
          variantUrl: `/product/${product.slug}/${variant.variantSlug}`,
